...
public class SeventhParticleState : IParticleState 
{
	...
	private bool checkEvol;																// reset collision timer, take hit timer
		...
		if (canCollide) {																	// if collision allowed and player is not stunned
			...
			if (other.gameObject.CompareTag ("Player") && !psp.inLightworld) {					// colide with player and not in light world
				...
				if (!pspOther.stunned) {															// if not stunned player
					...
					if ((pspOther.state != 7) && (psp.evolC > Mathf.Abs (pspOther.evolC))) {			// if abs player evol is lower
					...
					else if ((pspOther.state == 7) || (psp.evolC <= Mathf.Abs (pspOther.evolC))) {		// else player abs is higher
	...
	public void ToOtherWorld(bool toLW, int tst8, bool tl, int ts)
	{
		...
		psp.ChangeWorld(toLW, 7, tst8, tl, shape, ts);									// trigger transition effects
		//ParticleStateEvents.toZero += psp.TransitionToZero;								// flag transition in delegate
	}

	public void ToZero(bool toLight)
	{
		psp.TransitionTo(7, 0, isLight, toLight, shape, 0);								// trigger transition effects
		//ParticleStateEvents.toZero += psp.TransitionToZero;								// flag transition in delegate
	...
	{
		psp.TransitionTo(7, 1, isLight, toLight, shape, 0);								// trigger transition effects
	...
	{
		psp.TransitionTo(7, 2, isLight, toLight, shape, 0);								// trigger transition effects
	...
	{
		psp.TransitionTo(7, 3, isLight, toLight, shape, 0);								// trigger transition effects
	...
	{
		psp.TransitionTo(7, 4, isLight, toLight, shape, 0);								// trigger transition effects
	...
	{
		psp.TransitionTo(7, 5, isLight, toLight, shape, toShape);						// trigger transition effects
	...
	{
		psp.TransitionTo(7, 6, isLight, toLight, shape, toShape);						// trigger transition effects
	...
	{
		psp.TransitionTo(7, 7, isLight, toLight, shape, toShape);						// trigger transition effects
	}

	public void ToEighth(bool toLight, int toShape)
	{
		psp.TransitionTo(7, 8, isLight, toLight, shape, toShape);						// trigger transition effects
	...
	{
		psp.TransitionTo(7, 9, isLight, toLight, shape, toShape);						// trigger transition effects
	...
	public void Evol()
	{
		...
		if (evol == 1f && !inLightworld) {													// to dark world first / from dark world
			...
			else if (deltaDark <= deltaLight) ToFirst(true);									// if lose more dark than light = to ldark world ight first
		}
			// to light world
		...
		// ninth
			// in dark world
		if (((evol >= 34f && evol < 55f) && !inLightworld)									// to dark world ninth / from dark world
			|| ((evol <= -34f && evol > -55f) && inLightworld)) {							// to light world ninth / from light world
