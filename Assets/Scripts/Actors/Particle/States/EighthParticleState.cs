		...
		if (canCollide) {																	// if collision allowed and player is not stunned
			...
			if (other.gameObject.CompareTag ("Player") && !psp.inLightworld) {					// colide with player and not in light world
				...
				if (!pspOther.stunned) {															// if not stunned player
					...
					if ((pspOther.state != 8) && (psp.evolC > Mathf.Abs (pspOther.evolC))) {			// if abs player evol is lower
					...
					else if ((pspOther.state == 8) || (psp.evolC <= Mathf.Abs (pspOther.evolC))) {		// else player abs is higher
				...
				if (!pspOther.stunned && (pspOther.lightworld == psp.inLightworld)) {			// if particle and not stunned particle in same world
	...
	public void Evol()
	{
		...
		if ((evol <= -21f && evol > -34f) && !inLightworld) {								// to light world eighth / from dark world
			...
			// in light world
				// same state
			// to dark world
		else if ((evol >= 13f && evol < 21f) && inLightworld) {								// to dark world eighth / from light world
