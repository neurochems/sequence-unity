...
public class FifthParticleState : IParticleState 
{
	...
	public void Evol()
	{
		...
		// sixth
		if ((evol >= 8.0f && evol < 13.0f) && !lightworld) {								// evolve to dark world sixth within dark world
		...
		if ((evol <= -21f && evol > -34f) && !lightworld) {									// devolve to light world eighth from dark world
			if (deltaDark <= deltaLight) ToOtherWorld(true, 5, 8, true);						// if lose more dark than light = to light world light eighth
			else if (deltaDark > deltaLight) ToOtherWorld(true, 5, 8, false);					// if lose more light than dark = to light world dark eighth
		}
