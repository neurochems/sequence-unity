...
public class ThirdParticleState : IParticleState 
{
		...
		if (canCollide) {																// if collision allowed and player is not stunned
			...
			if (other.gameObject.CompareTag ("Player") && !psp.inLightworld) {				// colide with player and not in light world
				...
				if (!pspOther.stunned) {														// if not stunned player
					...
					if ((pspOther.state != 3) && (psp.evolC > Mathf.Abs (pspOther.evolC))) {			// if abs player evol is lower
					...
					else if ((pspOther.state == 3) || (psp.evolC <= Mathf.Abs (pspOther.evolC))) {		// else player abs is higher
	...
	public void ToOtherWorld(bool toLW, int tst8, bool tl, int ts)
	{
		...
		psp.ChangeWorld(toLW, 3, tst8, tl, 0, ts);										// trigger transition effects
	...
	public void Evol()
	{
		...
		if ((evol >= 5f && evol < 8f) && !inLightworld) {									// to dark world fifth / from dark world
			...
			else if (!isLight && (deltaDark <= deltaLight)) ToFifth(true, 2);					// if dark & gain more light than dark = to light square fifth
			else if (isLight && (deltaDark > deltaLight)) ToFifth(false, 0);					// if light & gain more dark than light = to dark circle fifth
			else if (isLight && (deltaDark <= deltaLight)) ToFifth(true, 2);					// if light & gain more light than dark = to light square fifth
		}
			// to light world
