...
public class FifthPlayerState : IParticleState 
{
	...
	public void Evol() 
	{
		...
		/*if ((evol <= -5f) && (evol > -8f)) {																// to light world fifth
			if (deltaDark <= deltaLight) ToFifth(true, );														// if lose more dark than light = to light fifth
			else if (deltaDark > deltaLight) ToFifth(false);													// if lose more light than dark = to dark fifth
		}*/
