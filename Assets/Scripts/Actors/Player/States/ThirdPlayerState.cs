...
public class ThirdPlayerState : IParticleState 
{
	...
	public void ToZero()
	{
		psp.TransitionToZero(3, psp.self);										// trigger transition effects
	...
	{
		psp.TransitionToFirst(light, 3, psp.self);								// trigger transition effects
	...
	{
		psp.TransitionToSecond(light, 3, psp.self);								// trigger transition effects
	...
	{
		psp.TransitionToFourth(light, 3, psp.self);								// trigger transition effects
	...
	public void Evol()
	{
		...
		if (psp.evol <= 0f) {																// to dead (if evol < 0)
			...
			if (deltaDark <= -1.5 && deltaDark >= -2) ToLightZero ();							// if lose dark = to light
			else if (deltaLight <= -1.5 && deltaLight >= -2) ToDarkZero ();						// if lose light = to dark
		}
