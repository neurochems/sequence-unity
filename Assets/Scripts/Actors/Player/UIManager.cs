...
*/
	...
	// philosophy \\
		...
		if (psp.lightworld) {
			...
			if (GetStateElapsed () > 20.0f) {																// 10 sec into being light world
			...
			if (GetStateElapsed () > 35.0f) {																// 20 sec into being light world
	...
	// tips \\
		...
		// tip propel 2
		if (!psp.timeCheck && !so.inMainMenu && GetPlaytimeElapsed () >= 60.0f) {							// 60 sec into playable/not-menu game
