...
public class OrthoSmoothFollow : MonoBehaviour {
	...
	void Update () {
		...
		if (rbPlayer.velocity.magnitude <= 1.0f) 																		// if velocity < 1
			transform.position = Vector3.SmoothDamp (transform.position, player.position, ref velocity, smoothTime * returnSpeed);	// perform smoothdamp on camera position back to player at centre
